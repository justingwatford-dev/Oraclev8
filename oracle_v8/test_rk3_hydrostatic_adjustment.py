...
def make_config_with_pgc() -> OperatorConfig:
    ...
    _unsafe_pgc_override=True bypasses the production guard that prevents
    this combination.  This is intentional: the test exists to prove the
    instability, not to run a valid simulation.
    """
    return OperatorConfig(
        equation_set=LH82AnelasticEquationSet(),
        staggering=LorenzStaggering(),
