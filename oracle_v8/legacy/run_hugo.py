...
reproduce the storm-agnostic validation and MUST NOT be used for results.
    ...
    USE INSTEAD:   python -m oracle_v8.run_storm Hugo
...
Oracle V8 — Hurricane Hugo (1989) Track Simulation
====================================================
First historical TC track run.  Initialized from HURDAT2 at
September 21, 1989, 00 UTC (~28 h before SC landfall).

Grid resolution note (paper limitation, documented):
    Hugo observed Rmax = 17 nm (31 km) = 2×dx on 15.6 km grid.
...
# Domain
# ---------------------------------------------------------------------------
# BIG_DOMAIN (V8.5.1): Hugo's 32.5N landfall sits at y~1.8-2.0 Mm in a 2000km
# domain — INSIDE the reversed-beta taper zone (>1600km).  So the validated
# 48.1km was itself taper-contaminated.  Doubling to 4000km/256 (dx unchanged)
# pushes the taper to y>3200km; Hugo's landfall (~2833km) is then clear — matching
# Katrina's clean big-domain config.  Requires the cap (256^2 uncapped blows up).
BIG_DOMAIN = True
if BIG_DOMAIN:
    Lx = Ly = 4_000_000.0
    nx = ny = 256
...
TAPER_START_FRAC = TAPER_START_M / R_ENV_M   # = 0.40 at R_env = 500 km
# Storm-agnostic stack — IDENTICAL to run_katrina (the point of re-validation):
WIND_TAPER  = True      # bound the Holland vortex at R_ENV_M (else it fills domain)
VMAX_CAP_MS = 70.0      # m/s perturbation-wind ceiling (None disables)
TAU_CAP     = 300.0     # s  cap relaxation timescale
DT         = 30.0        # s
N_STEPS    = 4320        # 36 h (V8.6: true init removes the legacy +4.2h head
                         #        start — crossing expected ~t+26-28h; 36h gives
                         #        margin so a slow run doesn't miss the threshold)
...
def main() -> int:
    ...
    print(f"  Rmax obs:     {s['Rmax_m']/1000:.1f} km (HURDAT2)")
    ...
    print(f"  Obs landfall: {s['landfall_lat']}°N, {s['landfall_lon']}°W "
          f"t+{s['landfall_time_h']:.0f}h")

    # ---- ERA5 DLM steering — load FIRST so t=0 values seed the vortex --------
    # The initial background flow is baked into the HollandVortex wind field.
    # The loop below then relaxes that background toward ERA5 DLM at the model
    # storm position, shifting the state and all perturbation-relative
    # references in lockstep.
    print("\nLoading ERA5 DLM steering winds...")
    try:
        era5     = ERA5Steering.load(storm="hugo")
        era5.print_summary()
    ...
    prebal_integrator = RK3Integrator(config=prebal_config, base=HugoBase())
    ...
    integrator = RK3Integrator(config=config, base=HugoBase())
    base_w     = wrap_base(HugoBase())
    # Intensity cap — per-step loop relaxation (as run_katrina/Ivan).  The cap
    # reference is perturbation-relative and is advanced in lockstep with the
    # live steering background in the loop below.
    cap_comp = (IntensityCapComponent(v_cap=VMAX_CAP_MS, tau=TAU_CAP,
                                      u_env=u_env_t0, v_env=v_env_t0)
                if VMAX_CAP_MS is not None else None)
    if cap_comp is not None:
        print(f"  Intensity cap: ACTIVE  |V'| -> {VMAX_CAP_MS:.0f} m/s (tau={TAU_CAP:.0f}s)")
    ...
    # Live background steering — relaxed toward the local DLM each diag step,
    # held in lockstep with the state background and the Coriolis/cap reference.
    ...
    for n in range(N_STEPS):
        ...
        if n % DIAG_EVERY == 0:
            ...
            # ---- Mean-flow relaxation (time-varying lockstep steering) ------
            # Sample ERA5 DLM at the storm's CURRENT position/time and relax the
            # background toward it, shifting the state AND the Coriolis/cap
            # reference by the SAME Δ (uniform shift → divergence/vorticity-free →
            # vortex untouched).  Replaces the old frozen-t0 + reference-only rule.
            if USE_ERA5:
            ...
            print(f"  {t_h:>6.1f}  {lat_c:>7.2f}  {abs(lon_c):>7.2f}  "
                  ...
                  f"{steer_tag} [{trk_method[:3]}]{tag}  "
                  f"θraw={lat_raw:.2f}N/{abs(lon_raw):.2f}W")
