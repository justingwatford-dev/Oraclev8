...
August 28, 2005, 00 UTC (~35 h before LA landfall).

Grid resolution note (paper limitation, documented):
    Katrina observed Rmax = 25 nm (46 km); run uses 75 km (5×dx) for stability.
...
from oracle_v8.storm_data import KATRINA
...
# Domain
# ---------------------------------------------------------------------------
# BIG_DOMAIN: the decisive taper test (Gemini). The reversed-β taper sits in the
# outer 20% of the y-domain; with Ly=2000 km the storm's 500 km outer circulation
# reaches it by mid-run and gets an equatorward push (the +8.6 h along-track lag,
# which capping intensity did NOT fix because the reach is set by R_env, not Vmax).
# Doubling the domain to 4000 km (keeping dx=15.6 km → nx=ny=256) pushes the taper
# zones to y<800 / y>3200 km; the storm (centre at 2000 km, reaching ~29°N≈2556 km,
# outer edge ~3056 km) never touches them. PREDICTION: the along-track lag vanishes.
# Needs the intensity cap ON (4× cells; the uncapped 256² run blew up). ~4× runtime.
BIG_DOMAIN = True
if BIG_DOMAIN:
    Lx = Ly  = 4_000_000.0
    nx = ny   = 256
...
# β-drift calibration (V8.7): harness gate-beta showed the β-drift (magnitude AND
# aim) is set by the taper-START radius = taper_start_frac · R_ENV_M.  Default 0.5
# -> 250 km -> 2.7 m/s @ 353° (westward only 0.33 m/s: the V8.6 "westward deficit"
# was a MIS-AIM, not under-magnitude).  200 km -> 2.49 @ 343°, westward 0.73 m/s --
# the in-band point that maximizes the westward component before the core limit.
# ONE shared value (storm-agnostic); set from β-drift physics, NOT from landfall.
# The ~8° poleward residual is fundamental (gyre resolution / NU4), not tunable here.
TAPER_START_M    = 200_000.0
TAPER_START_FRAC = TAPER_START_M / R_ENV_M   # = 0.40 at R_env = 500 km
WIND_TAPER = True
DT         = 30.0        # s
N_STEPS    = 5760        # 48 h — Run 7 reached 28.43°N at 41.5h (29.1°N needs ~6h more)
...
def main() -> int:
    ...
    try:
        era5     = ERA5Steering.load(storm="katrina")
        era5.print_summary()
        USE_ERA5 = True
        if TIME_VARYING_STEER:
            ...
            print(f"  ERA5 steering: ACTIVE  (frozen track-mean DLM — Katrina "
                  f"v_env varies 1.3→6.1 m/s; t=0 alone underrepresents)")
    ...
    state = init.build_state(nx, ny, nz, Lx, Ly, KatrinaBase())
    ...
    prebal_integrator = RK3Integrator(config=prebal_config, base=KatrinaBase())
    ...
    integrator = RK3Integrator(config=config, base=KatrinaBase())
    base_w     = wrap_base(KatrinaBase())
            ...
            if _HAVE_DIAG and n > 0:
                try:
                    _dd = _vdiag.compute_diagnostics(
                        state, KatrinaBase(), x_c, y_c, dx, dx,
    ...
    # threshold_lat = 29.1°N for Katrina (unchanged from the Run 12-15 logs).
